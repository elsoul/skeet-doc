...
import { docHeaderNav, docMenuNav } from '@/config/v1navs'
...
const mainContentId = 'v1DocMainContent'
                    ...
                    <nav className="space-y-1 px-2">
                      {docMenuNav.map((item) =>
                        !item.href && item.children ? (
                          <div
                            key={`v1DocLayout Menu ${item.name}`}
                            className="pt-2"
                          >
                            ...
                            {item.children.map((nav) => (
                              <Link
                                key={`v1DocLayout Menu ${item.name} ${nav.name}`}
                                href={nav.href}
                                className={clsx(
          ...
          <div className="flex flex-grow flex-col overflow-y-auto break-words bg-white pt-5 scrollbar-hide dark:bg-gray-900">
            ...
            <div className="mt-5 flex flex-1 flex-col">
              <nav className="flex-1 space-y-1 px-2 pb-4">
                {docMenuNav.map((item) =>
                  !item.href && item.children ? (
                    <div key={`v1DocLayout Menu ${item.name}`} className="pt-2">
                      <p className="p-1 text-sm font-bold text-gray-400 dark:text-gray-300">
                        {t(item.name)}
                      </p>
                      {item.children.map((nav) => (
